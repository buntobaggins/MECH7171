...
Declaration: I, Walker Golembioski, declare that the following program was written by me/us.
...
void draw(int mshape, void *mshapeData){
   ...
   bool bRightCanReach = true;
   bool bLeftCanReach = true;
   POINT2D toolTipPos;
   ...
   if (shape == LINE){
      ...
      for(i=1;i<=(*pLine).NP;i++){
         ...
         bLeftCanReach &= sol.bLeftCanReach;
         bRightCanReach &= sol.bRightCanReach;
      }
      //check if we can draw the line
      if(bLeftCanReach || bRightCanReach){
         //get colour/speed
         ...
         for(i=1;i<=(*pLine).NP;i++){
            ...
            if(bLeftCanReach == true){
               rotateRobotJoints(sol.leftArm);
            } else if (bRightCanReach == true){
   ...
   if (shape == CUBIC_BEZIER){
      ...
      for(i=1;i<=(*pCB).NP;i++){
         t = (i-1.0)/((*pCB).NP-1.0);
         toolTipPos = getTooltipPos(shape, pCB, t);
         //IK
         reachState = inverseKinematics(toolTipPos,&sol);
         //pring points
         printPointData(i,(*pCB).NP,toolTipPos,sol,reachState);
         if (i == (*pCB).NP){
            printTableHBorder(BL,BR,TABLE_WIDTH);
         } else {
            printTableHBorder(CL,CR,TABLE_WIDTH);
   ...
   if (shape == TRIANGLE){
      ...
      for(n=0;n<=2;n++){
         ...
         for(i=1;i<=pPart.NP;i++){
            t = (i-1.0)/(pPart.NP-1.0);
            toolTipPos = getTooltipPos(LINE, &pPart, t);
            //IK
            reachState = inverseKinematics(toolTipPos,&sol);
            //pring points
            int index;
            ...
            case 1:
               ...
               break;
            case 2:
               index = i-1 + (*pTri).NP[0] + (*pTri).NP[1];
               break;
            default:
               break;
            }
            printPointData(index,pPart.NP,toolTipPos,sol,reachState);
            if (i == pPart.NP){
               printTableHBorder(BL,BR,TABLE_WIDTH);
            } else {
               printTableHBorder(CL,CR,TABLE_WIDTH);
   ...
   if (shape == RECTANGLE){
      ...
      //creat table header
      sprintf_s(str, sizeof(str), "RECTANGLE: PA = [%lg,%lg], PB = [%lg,%lg], NP1 = %zu, NP2 = %zu",(*pRect).P[0],(*pRect).P[1],(*pRect).NP[0],(*pRect).NP[1]);
      printTableHeader(TABLE_WIDTH,str);
      //create array to hold all the points
      POINT2D apoints[4];
      apoints[0] = (*pRect).P[0];
      ...
      for(n=0;n<=3;n++){
         ...
         for(i=1;i<=pPart.NP;i++){
            ...
            case 1:
               ...
               break;
            case 2:
               index = i-1 + (*pRect).NP[0] + (*pRect).NP[1];
               break;
            case 3:
               index = i-1 + (2*(*pRect).NP[0]) + (*pRect).NP[1];
               break;
            default:
               break;
            }
